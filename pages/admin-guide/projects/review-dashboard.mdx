...
When first landing on the review dashboard we can see a list of user contributions to the left. Once a submission is selected, it will show on the right.
The two panels can be changed in size by dragging the center handle.
![](</assets/Screenshot 2023-08-09 at 13.23.06.png>)


## Review List
The review list (left) shows all the contributions for this project. By default, it's sorted by last modified and the sorting can be changed to any one of the column headings by ascending or descending.
...
The list will also include in progress and assigned contributions, not all items in the list can be viewed on the submission (right) panel.
![](</assets/Screenshot 2023-08-09 at 12.11.02.png>)
***

<StatusBadgeAssigned /> This task that has been assigned to a user but has not been started, there is nothing to view yet

<StatusBadgeProgress /> This task has been started, the user may have a draft, it cant be viewed by the reviewer

<StatusBadgeSubmitted /> This task is ready for review and has been submitted by a user
...
## Submission Panel
The selected submission can be viewed and actioned on the submission panel (right) with the addition of some useful information such as the assignee, status and resource titles. The submissions can be navigated back and forth by the arrows in the top corner.
...
**Making a correction** is intended for the reviewer to make ad hoc changes to the submission that aren't worth sending back to the user, such as selling mistakes. Once the correction as been made the reviewer can approve the submisison

**Requesting changes** is for bigger changes or corrections. It will mark the contribution as in 'changes requested' and notify the user, like a rejection the reviewer has the chance to write a message
...
Lastly there is a focus mode under 'View options' button that allows th reviewer to see the submission panel as fullscreen with all the same actions and information as list mode. Using the navigation arrows the reviewer can quickly focus on getting through many reviews.
